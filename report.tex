...
% NYCU Spring 2026
% Student ID: [YOUR_STUDENT_ID]  ← REPLACE THIS
% ─────────────────────────────────────────────────────────────────────────────
...
% ── Title Header ──────────────────────────────────────────────────────────────
\title{\vspace{-2cm}\textbf{NYCU Spring 2026 Introduction to AI\\ Homework 3: Reinforcement Learning Report}}
\author{
    \textbf{Student Name:} [YOUR_NAME] \qquad \textbf{Student ID:} [YOUR_STUDENT_ID] \\
...
\label{sec:dqn}
We implemented a Deep Q-Network agent from scratch in PyTorch, incorporating Double DQN updates to mitigate value overestimation. The main modules we wrote are:
...
Our SB3 models use default hyperparameter sets tailored for stability, using parallel environments ($N_{\text{envs}}=8$) for PPO to speed up Atari training on our NVIDIA RTX 3090.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    PPO (SB3)          & 141.89  & 93.05  & No  \\
    DQN (SB3)          & 236.58  & 49.78  & Yes  \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Observations:}
In LunarLander-v3, DQN easily solved the environment within the 500k budget, reaching a high mean reward of $+236.58$. On the other hand, PPO struggled to cross the Solved threshold, ending at $+141.89$. This indicates that off-policy sample reuse in DQN is highly effective for low-dimensional physical control tasks, whereas PPO's on-policy nature is less sample-efficient and would require more steps or better hyperparameter tuning to converge.
...
This assignment provided several valuable practical insights into reinforcement learning:

\begin{itemize}
  \item \textbf{Continuous State Control vs. Pixel Control:} 
    LunarLander (8 states) was quickly solved by DQN, demonstrating that low-dimensional vector states are extremely conducive to fast learning. In contrast, Atari Pong (pixel inputs) is orders of magnitude more complex. Training CNN feature extractors from scratch alongside policy learning is slow, unstable, and computationally demanding.
